...
func TestDay5(t *testing.T) {
	...
	for _, line:=range lines {
		...
		for _, rowData := range tokens[0:len(tokens)-3] {
			rowMin, rowMax = findRow(rowData, rowMin, rowMax)
		}
		for _, colData := range tokens[len(tokens)-3:] {
			colMin, colMax = findCol(colData,colMin, colMax)
		}
		id:=rowMin*8+ colMin
		if id > max {
			max = id
...
func findRow(place string, min, max int) (int, int) {
	if place == "F" {
...
func findCol(place string, min, max int) (int, int) {
	if place == "L" {
		return min, (max+min)/2
	}
	return ((max+min)/2)+1, max
